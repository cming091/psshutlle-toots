...
class ManualMerge(Base):
    def __init__(self, data):
        super(ManualMerge,self).__init__(data)

    def triggerBcr(self):
        url = '{}/api/trigger/merge'.format(CONF['baseUrl'])
        data = {
            "warehouseCode": self.data['warehouseCode'],
            "regionCode": self.data['regionCode'],
            "now": '2020-10-10+11%3a12%3a13'
        }
        res = RequestApi.sendReuest('triggerBcr', 'POST', url, data, headers=self.headers).json()
        logger.info('[{} triggerBcr: res:{}]'.format(self.data['sName'], res))
